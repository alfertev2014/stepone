...
Ptr ByteArrayFunctions::populateContext(const Ptr &a) const
{
    ...
    ctx = Context::make(aserfloat, Ptr::of<BaseMacro>(Function<SerializeUnOp<float> >()), ctx);
    ...
    ctx = Context::make(agetfloat, Ptr::of<BaseMacro>(Function<ByteArrayGetBinOp<float> >()), ctx);
