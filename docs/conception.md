...
Существующие языки созданы для своих задач. Большинство из них предназначены для написания кода вручную. Если в дизайне языка встречается какое-то неудобство, то оно пытается решиться через IDE или просто игнорируется и смиренно принимается. Большинство неудобств связаны с синтаксисом, с его негибкостью и нерасширяемостью. Они влекут обычно к костылям и велосипедам.

Многие языки предоставляют мало информации времени компиляции. Синтаксис их сложен, семантика тоже описывается сложными правилами. Из этого следует, что сделать компилятор или IDE с удобными визуальными тулзами - это целое таинство. Более того, всё, что есть в компиляторе или IDE, не достаточно гибко поддаётся настройке и кастомизации.


## Цели языка. Назначение, сферы применения.
